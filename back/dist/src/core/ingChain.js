...
class IngChain {
    constructor(chain, block, transaction, unspent, accounts) {
        this.chain = chain;
        this.block = block;
    ...
    mineBlock(account) {
        ...
        const coinbase = this.transaction.createConinbase(account, latestBlock.height);
        const newBlock = this.block.createNewBlock(latestBlock, [coinbase, ...transactions], adjBlock);
    ...
    sendTransaction(receipt) {
        ...
        if (!isValid)
            throw Error(`sendTransaction : 올바르지 않은 영수증입니다.`);
    ...
    addBlockInChain(receivedBlock) {
        ...
        console.log(`Ingchain - success add block: 블록 내용이 체인에 추가되었습니다.`);
        return true;
    }
    ...
    replaceChain(receivedChain) {
        ...
        if (rclb.height <= cclb.height) {
            ...
            console.log(`replaceChain: 현재 갖고 있는 블럭의 길이가 더 길거나 같습니다.`);
            return;
        }
